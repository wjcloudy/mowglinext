...
- Fastest path: GUI → **Settings → Docking** → **Start dock calibration** (one-click). With the robot on the dock, charging, and RTK-Fixed, it first captures the dock **position** right there from the raw GPS antenna (it waits up to 90 s for enough RTK-Fixed samples), then reverses in a straight line, fits the dock **heading** from the GPS course and saves position + heading together. It then re-docks as a confirmation only: that re-dock still steers by the OLD dock pose — the navigation stack loads the new one at its next restart — so if the old pose was wrong (or there was none) it may stop short of the charger. The result then reads "calibrated — but the robot is NOT on the dock": restart `mowgli-ros2`, then send HOME. If the position could not be captured, only the heading is saved and the result says so. The blade stays off throughout.
...
- Every dock-**position** writer requires the robot to be physically on the dock (firmware reporting `is_charging`) with a fresh, accurate GPS fix (the calibration is no exception: its position is captured on the dock, charging; only the final write, which adds the heading, happens off the dock) — that includes the manual **Set dock pose** button in Settings → Sensors and dragging the dock marker in the map editor. `map_server_node` rejects an off-dock capture.
