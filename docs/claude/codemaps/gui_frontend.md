...
| `src/types/irrisense.ts`, `src/types/remoteAccess.ts`, `src/types/notifications.ts`, `src/types/mapbox-gl-draw.d.ts`, `src/global.d.ts`, `src/vite-env.d.ts` | 76/60/60/72/12/1 | IrriSense DTOs; remote-access DTOs + `remoteAccessVerdict`; push-notification DTOs; ambient typings |
...
| `map/components/` (11 files) | 52–442 | `MapToolbar`, `MapToolbarMobile`, `MapEditorToolbar`, `AreasListPanel`, `TrackedObstaclesPanel`, `ObstacleProposalsPanel` (PENDING dig proposals from `map.working_area[].proposed_obstacles`: Accept → `promote_obstacle{pending_id}`, Reject → `discard_obstacle`; flattening in `map/utils/obstacleProposals.ts`; drawn dashed by `MapPage` `renderProposalLayers`, kept OUT of the editable feature set), `NewAreaModal`, `EditAreaModal`, `ImportOpenMowerModal`, `MapOffsetPanel`, `ShapePickerDropdown`, `JoystickOverlay` |
...
| Section components: `HardwareSection` `DriveMotorSection` `NtripSection` `PositioningSection` `SensorsSection` `LocalizationSection` `MowingSection` `DockingSection` `BatterySection` `SafetySection` `ObstaclesSection` `NavigationSection` `RainSection` + `IrriSenseSection` (both rendered by the one "weather" section — old `?section=rain|irrisense` links map to it) `LedsSection` `RemoteAccessSection` `NotificationsSection` `AdvancedSection` `DisplayModeSection` `LogTimeZoneSection` | 64–964 | One per `SettingsSection` id, except `appearance`, which renders `DisplayModeSection` + `LogTimeZoneSection` (18 components for 17 ids); `DriveMotorSection` (964) also hosts PID/FF tuning runs |
| `settingsFieldGroups.ts` / `SettingsFieldCard.tsx` | 160/125 | DECLARATIVE settings: a group is a list of `{key, kind, min/max/step, unit}` specs; the card renders it with the overridden-dot/reset label. `SettingsPage.renderFieldCards` appends groups under a section and `SECTION_DEFINITIONS` claims the keys via `groupKeys(...)`. To expose a new template key: schema entry (default = template) → a spec in a group → `settingsFields.<key>.label/tooltip` in en+fr. `SettingsFieldCard.test.tsx` fails on a missing en/fr string; `pkg/api/settings_field_groups_test.go` fails on a group key with no schema default (Go side, because the Docker web build typechecks tests with only `web/` in context — never import `asserts/` from `web/src`) |
...
`/mowglinext` · `/map` · `/schedule` · `/diagnostics` (incl. `?tab=parameters`) · `/statistics` · `/settings` · `/logs` · `/onboarding` (`/parameters` = redirect) — all lazy, all children of `AppShell`. `/concept` exists only in dev builds (`main.tsx` L29–32). `/` redirects to `/mowglinext` (`AppShell.tsx` L88–93).
...
- mowglinext#637 phase 3: a mowing area's ROS array index (`working_area_indices`) is NOT a stable identity — the area list is rebuilt wholesale (clear + re-add) on any edit save, so it can shift live, mid-session. Anything that captures an area selection and acts on it LATER than the render that captured it (`MapPage.tsx` `startSelectedArea`, `obstacleAreaIndex`/`obstacleAreaNames`) must resolve the CURRENT index from the area's stable id (`utils/mapAreaIndex.ts` `mowingAreaIndexById`, stamped as `properties.source_working_area_id` in `MapPage.tsx` `buildFeatures`/`useMapEditing.ts` `buildLabels`) at the moment it fires, not from a position captured earlier. `useMapFiles.ts` `handleSaveMap` must also round-trip each area's existing id in the save payload (`MowgliMapArea.id`) or map_server cannot preserve it across the rebuild, silently re-minting ids (and discarding BT coverage-resume progress) on every save.
