...
| One-click dock calibration (CalibrateDock action / GUI start service / status topic) | `ros2/src/mowgli_localization/src/calibrate_imu_yaw_node.cpp` `run_dock_calibration_core()`, `capture_dock_antenna_with_wait()`, `persist_dock_pose()`, `persist_dock_via_map_server()`, `dock_start_cb()`; the `set_docking_point` requests + the success/failure verdict and operator messages are pure in `include/mowgli_localization/dock_persist_plan.hpp` (`DockPoseStep`, `DockYawStep`, `DecideDockVerdict`; `test_dock_persist_plan`) |
...
| Legacy blocking IMU-yaw / pitch-roll / mag figure-8 calibration service | `calibrate_imu_yaw_node.cpp` `calibrate_cb()` L1679, legacy dock pre-phase `run_dock_yaw_drive()` (persists through map_server like the one-click flow: on-dock antenna capture, then `persist_dock_pose()`) |
| Where dock_pose_x/y/yaw get written to `mowgli_robot.yaml` | `ros2/src/mowgli_interfaces/include/mowgli_interfaces/robot_yaml_scalar.hpp` (`UpdateDockPose`); ONLY caller file: `mowgli_map/src/area_manager.cpp` (`on_set_docking_point`, datum migration). This package calls `/map_server_node/set_docking_point` instead |
...
| `/calibrate_imu_yaw_node/calibrate_dock` | `mowgli_interfaces/action/CalibrateDock` | action server (L314): WAIT_RTK → CAPTURE_POSITION (on the dock, charging, RTK-Fixed: map_server averages the RAW antenna, bounded 90 s wait) → REVERSING → CHECK_COG → PERSIST (ONE write off the dock: motion yaw + pending antenna; yaw-only fallback) → REDOCKING → VERIFY_CHARGE (pure confirmation — steers on the OLD pose, may stop short, never fails a saved calibration). `success` = yaw AND position saved; it can carry `RETRY_NO_CHARGE_ON_REDOCK` (robot NOT on the dock — warning). Every terminal outcome is logged (`finish`), not only published on the volatile status topic | (no GUI client; GUI uses the façade below) |
...
| `/map_server_node/set_docking_point` | `mowgli_interfaces/srv/SetDockingPoint` | **client** (L310), ONE write per run, off the dock: `use_pending_antenna=true, yaw_source=MOTION` (x, y, yaw), falling back to `preserve_position=true, yaw_source=MOTION` (yaw only) when no capture is pending. Never `use_gps_position` (rejected off the dock) | one-click AND legacy `~/calibrate` persist path (`persist_dock_pose`, `dock_persist_plan.hpp`) |
| `/map_server_node/capture_dock_antenna` | `std_srvs/Trigger` | **client**: called on the dock before the reverse leg, retried until map_server's RTK-Fixed sample window is full (≤ 90 s) | `capture_dock_antenna_with_wait()` |
...
- **This package no longer writes `dock_pose_*` to the yaml at all.** Three file writers were first collapsed onto `robot_yaml_scalar::UpdateDockPose`; now the only caller file is `mowgli_map/area_manager.cpp`. Both the one-click action and the legacy `~/calibrate` dock pre-phase go through `/map_server_node/set_docking_point` (`persist_dock_pose()`). The legacy pre-phase used to splice the file with `x0/y0` read from `/gps/absolute_pose` — a position lever-arm-corrected with the fused yaw, which on the dock is gauge-pinned to the STORED dock yaw, and which silently degrades to the raw antenna when TF is late. `/gps/absolute_pose` is still used there, but only for the reverse leg's displacement and line fit. The write happens as soon as the COG/line-fit yaw is validated — NOT after the re-dock, which is a pure confirmation pass (`docking_server`/`gps_dock_detection_node` read `dock_pose_x/y/yaw` as ROS params at container startup only) — and the POSITION half is captured BEFORE the reverse, on the dock (see `dock_persist_plan.hpp` for the two orderings that failed).
