...
| Handle a new Host→Firmware packet | `firmware/stm32/ros_usbnode/src/ros/ros_custom/cpp_main.cpp` `init_ROS()` (:1435, `mowgli_comms_register_handler` ×9) + a `static void on_*()` handler; check `MOWGLI_COMMS_MAX_HANDLERS` (`include/mowgli_comms.h:83`, 16) |
...
| Per-wheel velocity PI (gains, feedforward, anti-windup) | `cpp_main.cpp` `motors_handler()` :813 (`#if USE_WHEEL_PI` block :986-1101); power-on defaults `WHEEL_PI_KP_PWM_PER_MPS` 30 / `WHEEL_PI_KI_PWM_PER_MPS_S` 5000 / `WHEEL_PI_INT_MAX_PWM` 100 in `include/drive_tuning_defaults.h`; live gains applied by `apply_drive_params()`; PID core `include/pid.hpp` |
| Gyro yaw-rate loop (Option C) | `cpp_main.cpp:180-249` (power-on `YAW_PI_*_DEFAULT` in `include/drive_tuning_defaults.h`; `YAW_GYRO_LP_ALPHA` 0.30, `YAW_TRIM_SLEW_MPS_PER_CYCLE` 0.03, turn-exit/low-speed constants) and the loop body :858-965; runtime retune via `apply_yaw_params()` (protocol v7 yaw ids) |
...
| Emergency sensors, trip timeouts, play-button clear | `firmware/stm32/ros_usbnode/src/emergency.c` `EmergencyController()` :187, `emergency_set_timeouts()` (clamped to the `fw_param_catalog.h` envelope), `Emergency_SetState()` :112; defaults in `include/board_defaults.h:88-102` |
| Charge envelope / CC-CV state machine / SOC | `firmware/stm32/ros_usbnode/src/charger.c` `ChargeController()` :211, `charger_set_charge_limits()` (clamped to the `fw_param_catalog.h` envelope, top = 29.4 V / 1.2 A); `MAX_CHARGE_VOLTAGE` 29.4 / `MAX_CHARGE_CURRENT` 1.2 in `include/board_defaults.h:44-49`; ADC + RTC-backup persistence `src/adc.c:244-248` |
...
| Onboard LIS3DH tilt interrupt threshold | `firmware/stm32/ros_usbnode/src/i2c.c` `sensor_config[]` INT1_THS (compiled default `IMU_ONBOARD_INCLINATION_THRESHOLD`, clamp 0x2C..0x40 → 0x38 in `include/board_defaults.h`); runtime value via `I2C_Onboard_SetInclinationThreshold()`, applied from `SENSOR_POLL` |
...
| Runtime parameters (protocol v7): ids, envelope, persistence | `include/fw_param_catalog.h` (stable ids + absolute `[min, max]`, `fw_param_coerce()`), `src/fw_params.c` (table, reports, commit), `include/fw_param_log.h` (append-only record format), `src/fw_param_store.c` (flash region + HAL erase/program); apply hooks `cpp_main.cpp` `apply_param_groups()`; boot load `main.c` `fw_params_init()` before `WATCHDOG_vInit()`; test `firmware/scripts/test_fw_params.py` |
...
| `test/README`, `lib/README`, `include/README` | 11, 46, 39 | PlatformIO placeholders; the one native test is `test/test_cmd_vel_safety.cpp` (`pio test -e native`), the rest is `firmware/scripts/test_*.py` |
| **Secondary trees** | | |
...
| 0x58 | `PKT_ID_SET_PARAM` / `pkt_set_param_t` | H→F | 9 | `on_set_param()` → `fw_params_set()` | `param_id` + float `value`; coerced into the id's envelope (`fw_param_catalog.h`), non-finite rejected; group applied from the main loop; answered by 0x13 |
| 0x59 | `PKT_ID_GET_PARAM` / `pkt_get_param_t` | H→F | 5 | `on_get_param()` | one id, or `FW_PARAM_ID_ALL` (0xFFFF) = every 0x13 + one 0x14 |
| 0x5A | `PKT_ID_PARAM_COMMIT` / `pkt_param_commit_t` | H→F | 4 | `on_param_commit()` | `magic == PKT_PARAM_COMMIT_MAGIC` (0xC5); persists the non-volatile set, skipped when unchanged |
| 0x13 | `PKT_ID_PARAM_VALUE` / `pkt_param_value_t` | F→H | 23 | `fw_params_service()` | status (OK/CLAMPED/UNKNOWN_ID/REJECTED), flags (PERSISTED/VOLATILE), applied value, default, envelope min/max; ≥5 ms apart |
| 0x14 | `PKT_ID_PARAM_STORE_STATUS` / `pkt_param_store_status_t` | F→H | 9 | `fw_params_service()` | `boot_source` (defaults / flash / erased at boot), `last_commit`, `records_left`, `param_count` |

0x54–0x57 (per-group SET_DRIVE_PID / YAW_PID / KINEMATICS / SAFETY_LIMITS) were retired in protocol v7 — never reuse them. Runtime parameters persist in flash: `fw_params_init()` (`main.c`, before `WATCHDOG_vInit()`) loads the newest valid record of the append-only log (`fw_param_log.h`, region in `fw_param_store.c`), erasing only at boot; `init_ROS()` applies every group, so the board runs the stored set before the host connects. The bridge still re-sends everything after each connect, then commits.

### Main loop (`src/main.c:588-679`, cooperative, no RTOS)
`chatter_handler` (1 s LED + reboot) → `motors_handler` (20 ms: snapshot → hard-stop decision → yaw loop → wheel PIs → anti-dig → `DRIVEMOTOR_SetSpeedSigned` → heartbeat check → `BLADEMOTOR_Set`) → `panel_handler` (100 ms) → `spinOnce` (`apply_param_groups` + `fw_params_service`: one flash word / one report per pass) → `broadcast_handler` (one packet group per pass) → `DRIVEMOTOR_App_Rx` → [`Perimeter_vApp`] → every 10 ms `ADC_input` + `ChargeController` → 1 s `StatusLEDUpdate` → 10 ms `WATCHDOG_Refresh` → 20 ms `DRIVEMOTOR_App_10ms` → 100 ms `BLADEMOTOR_App` → 200 ms buzzer → 10 ms `EmergencyController` (compiled out by `I_DONT_NEED_MY_FINGERS`).
...
### Host-side consumers
- `ros2/src/mowgli_hardware/src/hardware_bridge_node.cpp`: handshake (`send_config_request()` :2558, `fw_compatible_ = (fw_protocol_version_ == kMowgliProtocolVersion)` :2466 — mismatch blocks mowing), `send_drive_pid/yaw_pid/kinematics/safety_limits/imu_params()` (each a series of `send_param()` = SET_PARAM) + end-of-burst `send_param_commit()` / `send_get_param(FW_PARAM_ALL)`; reports → `FirmwareParamTracker` → latched `/hardware_bridge/firmware_params`; `heartbeat_rate` param (4 Hz).
...
- `TICKS_PER_M` / `MAX_MPS` / `WHEEL_BASE` compile-time → only the default when nothing is stored; the live values come from `mowgli_robot.yaml` via the bridge (SET_PARAM) and are persisted in flash. The hard limits are the `fw_param_catalog.h` envelope, not `MAX_MPS`.
- New/changed runtime parameter → `fw_param_catalog.h` (new id, never reuse) + default in `fw_params.c` `fw_params_compiled_default()` + apply hook in `cpp_main.cpp` → host `FirmwareParamId` (`ll_datatypes.hpp`) + `firmware_params.hpp` name + bridge send → `test_fw_param_catalog.cpp`; envelope change → `charger.c`/`emergency.c`/`drivemotor.c` clamps + GUI `settingsFieldGroups.ts` bounds + schema text.
- Firmware image growth → must stay under `board_upload.maximum_size` in `platformio.ini` (the parameter log lives just above it).
...
- `MOWGLI_COMMS_MAX_HANDLERS` too small = silent "Firmware incompatible / vunknown" forever (history in `mowgli_comms.h:73-80`); `init_ROS()` registers 9.
...
- `on_*` handlers run in USB-RX interrupt context — no `debug_printf` on success paths (it starved the loop and tripped WWDG); `on_set_param` therefore only stores and the main loop applies.
...
- Encoder tick anomalies (reversal coast, controller counter reset) are handled in `DRIVEMOTOR_UpdateWheel`/`resolve_direction`; glitches above `drivemotor_max_ticks_per_frame()` (runtime `max_mps × tpm × 0.02 × 3`) are dropped, so a wrong runtime `ticks_per_meter` also changes what counts as a glitch.
