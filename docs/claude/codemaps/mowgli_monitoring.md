...
| Configure the broker (host/port/credentials the bridge connects TO) | GUI Settings → MQTT (`gui/web/src/components/settings/MqttSection.tsx`) writes `mqtt_enabled`/`mqtt_host`/`mqtt_port`/`mqtt_username`/`mqtt_password`/`mqtt_topic_prefix`/`mqtt_use_ssl`/`mqtt_home_assistant_discovery_enabled` into the installed `mowgli_robot.yaml` (Invariant 15); `full_system.launch.py` injects them into the node's `parameters=[...]`, layered after the package-share `mqtt_bridge.yaml` defaults (`mqtt_client_id`, `publish_rate`, not GUI-exposed) |
| Home Assistant discovery | `serialise_home_assistant_discovery()` creates one device-level payload at `homeassistant/device/<derived-id>/config`; it is retained, published on MQTT reconnect, and refreshed on `homeassistant/status=online`. The feature is opt-in through `mqtt_home_assistant_discovery_enabled`; disabling clears the retained record for the current prefix. |
...
| MQTT `<prefix>/pose` | JSON `{x,y,yaw}` from `/odometry/filtered_map` (fused, **map frame**), rate-limited to `publish_rate`; non-finite poses are dropped in `on_pose()` | out | QoS 1, retain=false | position + heading for a map consumer, smoother than `<prefix>/gps` |
| MQTT `<prefix>/area_boundary` | JSON `{datum_lat,datum_lon,areas:[{index,name,boundary,obstacles}],dock?:{x,y,yaw}}`, map-frame `[x,y]` metre offsets; `dock` (from `dock_pose_x/y/yaw`, read at startup) is omitted while they are all still 0/0/0 (`make_dock_pose()`) | out (retain=true), polled every 10 s via `on_timer()` | QoS 1 | `serialise_area_boundaries()`; only republished when the geometry changes — see [`docs/MQTT_CONTROL.md`](../../MQTT_CONTROL.md) |
...
| MQTT `<prefix>/host` | JSON `{ip}` — the bridge's own LAN IP (`detect_local_ip()`, a UDP-connect routing-table lookup, no traffic sent) | out, published once on the first successful connect (retained, so a reconnect need not repeat it) | QoS 1, retain=true | lets a consumer (e.g. Home Assistant) link to the mower's own GUI at `<ip>:4006` without the operator entering it; not published at all when there is no default route |
...
| mqtt | `publish_rate` | 1.0 Hz (clamped to [0.01,100]); rate limit for position/gps/status/power/rtk_status (latest value wins). The MQTT network loop is NOT tied to it — `net_timer_`, fixed 50 ms | :392, :395 | :22 — package default only, not GUI-exposed |
...
| mqtt | `home_assistant_discovery_enabled` | `false` | `declare_parameters()` | `mqtt_bridge.yaml` — **overridden** by `mowgli_robot.yaml.mqtt_home_assistant_discovery_enabled` |
...
- The MQTT network loop runs on its own 50 ms `net_timer_` (`create_timer()`), and `MosquittoMqttClient::spin_once()` keeps calling `mosquitto_loop(…, 0, 1)` while `mosquitto_want_write()` is true (bounded to 64 iterations). It used to be driven once per `publish_rate` tick, which let the outgoing QoS 1 queue grow faster than it drained: a 2026-09-21 field capture saw `<prefix>/high_level_status` reach the broker at ~0.57 msg/s against ~1 msg/s produced, i.e. HA showed a state that was 9+ min old after a 16-min run. `status`/`power`/`rtk_status` are coalesced to the latest value per `publish_rate` tick (like `position`/`gps`); `emergency` and `high_level_status` publish on every message.
