...
namespace gnss = mowgli_interfaces::gnss_status_utils;

TEST(GnssStatusAuthorityTest, RtkFixedStatusRemainsFixedForBehaviorTreeAndHardware)
{
  GnssStatus status;
  ...
  EXPECT_EQ(gnss::BehaviorTreeFixType(status), 4u);
  EXPECT_TRUE(gnss::BehaviorTreeRtkFixed(status));
  EXPECT_EQ(gnss::HardwareQualityPercent(status), 100u);
  EXPECT_EQ(gnss::AbsolutePoseFlags(status),
            static_cast<std::uint8_t>(AbsolutePose::FLAG_GPS_RTK |
                                      AbsolutePose::FLAG_GPS_RTK_FIXED));
}

TEST(GnssStatusAuthorityTest, RtkFloatStatusKeepsSharedIntermediateState)
{
  GnssStatus status;
  status.fix_valid = true;
  ...
  EXPECT_EQ(gnss::BehaviorTreeFixType(status), 3u);
  EXPECT_FALSE(gnss::BehaviorTreeRtkFixed(status));
  EXPECT_EQ(gnss::HardwareQualityPercent(status), 70u);
  EXPECT_EQ(gnss::AbsolutePoseFlags(status),
            static_cast<std::uint8_t>(AbsolutePose::FLAG_GPS_RTK |
