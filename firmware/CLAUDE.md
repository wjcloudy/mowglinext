...
The maintained branch is `codex/lfp-firmware`; `ADC_CHARGING_DMA` and
`CHARGE_DIAGNOSTICS` independently select acquisition/monitoring (default 0).
Four explicit LFP targets and frozen predecessor references are listed in LFP.md.
The opt-in `Yardforce500B_LFP_DIAG` DMA recorder alias, native test and non-halting dump
